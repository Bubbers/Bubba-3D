...
float4x4 convertMatrix(aiMatrix4x4 old);
...
bool SkinnedMesh::loadMesh(const std::string& fileName) {
	...
	if (!pScene) {
		...
		float4x4 mNew = inverse(convertMatrix(m));
...
void SkinnedMesh::initMats(const aiScene* pScene, const std::string& fileName) {
...
SkinnedMesh::Chunk::Chunk(std::vector<chag::float3>& positions,
...
GLuint SkinnedMesh::loadTexture(std::string fileName)
{
